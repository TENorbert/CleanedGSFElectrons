...
//  std::string folderInFile("recalibratedTimeHists/");   // >> if you want to see the recalibrated average times
  ...
  timingCalibDiffEB->Fit("fit","Q");
  float timingMeanEB = fit->GetParameter(1);
  float timingSigmaEB = fit->GetParameter(2);
  ...
  timingCalibDiffsEEM->Fit("fit","Q");
  float timingMeanEEM = fit->GetParameter(1);
  float timingSigmaEEM = fit->GetParameter(2);
  ...
  timingCalibDiffsEEP->Fit("fit","Q");
  float timingMeanEEP = fit->GetParameter(1);
  float timingSigmaEEP = fit->GetParameter(2);
